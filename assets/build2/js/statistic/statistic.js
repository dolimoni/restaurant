function init_daterangepicker() {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.status === "success") {
                    ...
                    updateSpendingGraph(data.report);
                    updatePaiementsGraph(data.report.orders);
                }
                else {
                    console.log('Error');
                }
    ...
    function updatePaiementsGraph(data){
        ...
        paymentData.push(data.report.ordersToPayAmount);
    ...
    function update_history_graph(data){
        ...
        var chart_plot_sale_data = [];
        var chart_plot_charge_data = [];
        $.each(sales, function (key, sale) {
            chart_plot_sale_data.push([new Date(sale['report_date']), sale['s_amount']]);
        });
        ...
        var min_xaxis=0;
        if(chart_plot_sale_data[0]){
            min_xaxis= chart_plot_sale_data[0][0];
        }
    ...
    function update_topConsomation_graph(data){
        ...
        $.each(products_cost, function (key, product) {
            ...
            productModel.find("p small").html(number_of_uses_lang+" : "+product["s_meal"]+" "+time_nbr);

                            ...
                            <div class="col-xs-12">
                                <div class="" role="tabpanel" data-example-id="togglable-tabs">
                                    ...
                                    <ul id="myTab" class="nav nav-tabs bar_tabs sm-hidden" role="tablist">
                                        <li role="presentation" class="active"><a href="#tab_orders" role="tab" id="profile-tab"
                                                                            data-toggle="tab" aria-expanded="false"><?= lang('history_of_orders') ?></a>
                                        </li>
                                        <li role="presentation" class=""><a href="#tab_products" id="home1-tab"
                                                                                  role="tab"
                                    ...
                                    <div class="col-md-3 col-sm-12 col-xs-12 md-hidden md-zero-height">
                                        <!-- required for floating -->
                                        <!-- Nav tabs -->
                                        <ul class="nav nav-tabs tabs-left">
                                            <li class="active"><a href="#tab_orders" data-toggle="tab" aria-expanded="false"><?= lang('history_of_orders') ?></a>
                                            </li>
                                            <li class=""><a href="#tab_products" data-toggle="tab" aria-expanded="true"><?= lang('products') ?></a>
                                            </li>
                                    ...
                                    <div id="myTabContent" class="tab-content col-md-12 col-sm-12 col-xs-12">
                                        ...
                                        <!--------------------------------------------Products Tab------------------------------------------------------>
                                        <div role="tabpanel" class="tab-pane fade" id="tab_products"
                                        ...
                                        <div role="tabpanel" class="tab-pane fade" id="tab_productsToOrder"
                                            ...
                                            <table class="data table table-striped table-bordered no-margin">
                                                <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th><?= lang('product') ?></th>
                                        ...
                                        <div role="tabpanel" class="tab-pane active fade in " id="tab_orders"
                                                ...
                                                <table id="datatable-responsive5"
                                                    ...
                                                    foreach ($orders as $order) {
                                                        ...
                                                        if($order['paid']==="true"){
                                                            ...
                                                            $impaidAmount+= (float)($order['ttc']-$order['advance']-$order['advance']-$order['discount']);
                                                        ...
                                                        <tr data-id="<?php echo $order['id']; ?>">
                                                            ...
                                                            <td><?php echo number_format($order['ttc'],2)?></td>
                                                            <td><?php echo number_format($order['advance'],2)?></td>
                                                            <td class="<?php echo $paidClass; ?>"><span><?php echo number_format($remain,2); ?></span></td>
                                                            <td><?php echo number_format($global_remain,2)?></td>
    ...
    $(document).ready(function () {
        var handleDataTableButtons = function () {
            ...
            if ($("#datatable-responsive5").length) {
                dataTableOrders=$("#datatable-responsive5").DataTable({
                    aaSorting: [[0, 'desc']],
                    "bInfo": false,
                    "columns": [
                        { "data": "id" },
...
<script src="<?php echo base_url('assets/build2/js/provider/main.js'); ?>"></script>
...
<script>
    ...
    $("input[type='text'][name='order_tva']").keyup(function(){
...
<script>
    $(document).ready(function () {
        $('#datatable-responsive5').on('click','a.deleteOrder', deleteOrder);
...
<script>
    $(document).ready(function () {
        function init_echarts() {
            ...
            if ($('#echart_pie').length) {
                ...
                $.each(javascript_statistics, function (key, value) {
                    var element={
                        value: parseFloat(value.s_od_totalPrice).toFixed(2),
                        name: ''+value.name+''
                    }
                    myData.push(element);
                });
        ...
        function handleOrdersData(data) {
            console.log('dataTableOrders',dataTableOrders);
            dataTableOrders.clear().draw();
            ...
            $.each(data.orders, function (key,order) {
                ...
                let paid = 'Impayée';
                let paidClass="orderImpaid";
                let remain=order['ttc']-order['advance'];
                if (order['status'] === "canceled") {
                    orderStatus = 'Annulée'
                } else if (order['status'] === "received") {
                    orderStatus = 'Reçue';
                ...
                if(order['paid']==="true"){
                    paidAmount+=parseFloat(order['ttc']).toFixed(2);
                    paid= 'Payée';
                    paidClass="orderPaid";
                }else{
                    impaidAmount+= parseFloat(order['ttc']-order['advance']-order['advance']-order['discount']).toFixed(2);
                    paidAmount+=parseFloat(order['advance']).toFixed(2);
                }
                global_remain+=remain;

                impaidAmount=global_remain;

                var editAction='<a class="btn btn-primary btn-xs editOrderModal" data-toggle="modal" data-target="#editOrderModal" data-id="'+order['id']+'">Modifier</a> ';
                var deleteticAction='<a data-id="'+order['id']+'" class="btn btn-danger btn-xs deleteOrder">Supprimer</a>';
                ...
                var row={
                    ...
                    "global_remain": global_remain,
                    "orderDate": dateTimeToDate(order.orderDate),
                    "paymentDate": dateTimeToDate(order.paymentDate),
                    "status": orderStatus,
                    "payment": '<span>'+paid+'</span>',
                    "actions": editAction+deleteticAction
                };
                var node = dataTableOrders.row.add(row).draw().node();
                $(node).find('td:eq(4)').addClass(paidClass);
                $(node).find('td:eq(9)').addClass(paidClass);
            });
        ...
        function init_daterangepicker() {
            ...
            var cb = function (start, end, label) {
                ...
                apiRequest(apiFilterOrders_url,myData,params,handleOrdersData);
            };

...
class model_employee extends CI_Model {
	...
	public function updateEmployee($id, $employee)
	{
		...
		$this->updateCurrentMonthSalary($id,$employee['salary']);

		$event=array(
		    'employee'=>$id,
		    'day'=>date('Y-m-d')
        );
		if(!$this->isPaid($id,date('Y-m-d'))){
            $this->automaticSalary($event);
        }
	}

	public function updateCurrentMonthSalary($id,$salaray){
        $this->load->model('model_util');
        $lastDay = $this->model_util->getLastDayInMonth(date("Y-m-d"));
        $this->db->where('paymentDate',$lastDay);
        $this->db->where('employee',$id);
        $this->db->where('paid','false');
        $this->db->update('salary',array('salary'=>$salaray));
    }
	public function createEvent($event)
	{
		...
		$employe=$this->get($event['employee']);
		$newMonthSalary=$this->createMonthSalary($event['employee'],$event['day'],$employe['salary']);

		//Si l'employée n'est pas payé et qu'on lui donne une avance un une absence
        if (!$this->isPaid($event['employee'],$lastDay) and
            ((strpos($event['remarque'], 'avance') !== false)
            or (strpos($event['remarque'], 'absence') !== false))) {
            $this->db->insert('employee_event', $data);
            $this->automaticSalary($event);
        }else{
            $response['status']='warning';
            $response['message']='Le salaire a été déjà payé';
        }

        if(((strpos($event['remarque'], 'avance') === false)
            and (strpos($event['remarque'], 'absence') === false))){
            $this->db->insert('employee_event', $data);

            $response['status']='success';
            $response['message']='L\'opération a été bien effectuée';
        }
        if($newMonthSalary){
            $this->automaticSalary($event);
            $response['status']='success';
    ...
    public function isPaid($employee,$date){
    ...
    public function createMonthSalary($employee,$date,$salary){
        $this->load->model('model_util');
        $lastDay=$this->model_util->getLastDayInMonth($date);
        $this->db->where('employee',$employee);
        $this->db->where('paymentDate',$lastDay);
